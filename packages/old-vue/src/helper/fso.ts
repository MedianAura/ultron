...
export function robocopy(sourceDir: String, destDir: String, fileMask: String, param: String[]) {
  ...
  async.each(
    aFileMask,
    function(mask: string, nextFile: any) {
      let aParam = [sourceDir, destDir, mask];
      ...
      let status = window
        ...
        .spawnSync('C:\\Windows\\System32\\robocopy.exe ', aParam, { windowsVerbatimArguments: true }).status;

    ...
    <div class="cellule-info">
        ...
        <div v-for="(convert, index) in listAction" :key="index" class="form-row">
          <div class="form-group col-md-12">
            ...
            <table class="table">
              <tr>
                <td>codec-in:</td><td>{{convert['codecIn']}}</td>
              </tr>
              <tr>
                <td>codec-out:</td><td>{{convert['codecOut']}}</td>
              </tr>
              <tr>
                <td>input-path:</td><td>{{convert['__inputPath']}}</td>
              </tr>
              <tr>
                <td>input-mask:</td><td>{{convert['inputMask']}}</td>
              </tr>
              <tr>
                <td>output-path:</td><td>{{convert['__outputPath']}}</td>
              </tr>
            </table>
          </div>
        </div>
...
export default class Convert extends Vue {
  ...
  @Getter('application/getSelectedVersion') selectedVersion!: VersionsType
  ...
  get listAction (): object[] {
    return this.steps[this.sequence]['options']['codec'].map((value: CodecParams) => {
      ...
      value['__outputPath'] = this.workPath + value['outputPath']
      return value
    })
  }

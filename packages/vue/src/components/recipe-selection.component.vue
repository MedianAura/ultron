<template>
  <div>
    <div class="form-row">
      ...
      <div class="form-group col-4">
        <button class="btn btn-primary" @click="selectPackage(selectedVersion)" :disabled="selectedGitVersion === ''"
                id="go-actions">Lancer
        </button>
      </div>
    </div>
    <div>
      <b-modal size="lg" id="anmodal" title="Actions à effectuer" :no-close-on-backdrop="true" :no-close-on-esc="true" :hide-header-close="true">
        <div>
          <div class="cellule-info">
            <div class="row">
              <div class="col-md-4"><h5>Dossier de travail:</h5></div><div class="col-md-8">{{selectedVersion.workPath}}</div>
            </div>
          </div>
        </div>
        <div>
          <form data-vv-scope="wizard">
          <component v-for="(actionComponent, index) in getActionComponents" :key="index" :sequence="index" v-bind:is="actionComponent"></component>
          </form>
        </div>
    ...
    <div>
      <b-modal ref="progressModal" id="pmodal" title="Avancement" hide-footer :no-close-on-backdrop="true" :no-close-on-esc="true" :hide-header-close="true">
        ...
        <b-btn v-show="progress.actionCompleted === true" id="btn-close-progress" size="sm" variant="primary" @click="closeProgress">
          Fermer
        </b-btn>
      </b-modal>
    </div>
    <div>
      <b-modal header-bg-variant="warning" centered ref="undefinedTasksModal" id="utmodal" title="Attention"  :no-close-on-backdrop="true" :no-close-on-esc="true" :hide-header-close="true">
